    ...
    if config.INFERENCE.EVALUATION_METHOD == "peak detection":
        ...
        for metric in config.UNSUPERVISED.METRICS:
            if metric == "MAE":
                ...
                print("=== Gaussian Process Uncertainty Analysis ===")
                from sklearn.gaussian_process import GaussianProcessRegressor
                from sklearn.gaussian_process.kernels import RBF, ConstantKernel as C
                import matplotlib.pyplot as plt

                # Select the appropriate HR arrays based on the evaluation method
                if config.INFERENCE.EVALUATION_METHOD == "peak detection":
                    X = predict_hr_peak_all.reshape(-1, 1)
                    y = gt_hr_peak_all
                elif config.INFERENCE.EVALUATION_METHOD == "FFT":
                    X = predict_hr_fft_all.reshape(-1, 1)
                    y = gt_hr_fft_all
                else:
                    raise ValueError("Invalid evaluation method for GP analysis")

                # Define and fit the Gaussian Process model
                kernel = C(1.0, (1e-3, 1e3)) * RBF(10, (1e-2, 1e2))
                gp = GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=10)
                gp.fit(X, y)
                y_pred_gp, sigma_gp = gp.predict(X, return_std=True)

                # Compute error metrics for the GP predictions (optional)
                MAE_GP = np.mean(np.abs(y_pred_gp - y))
                RMSE_GP = np.sqrt(np.mean(np.square(y_pred_gp - y)))
                print("GP Calibrated MAE: {:.2f}".format(MAE_GP))
                print("GP Calibrated RMSE: {:.2f}".format(RMSE_GP))
                print("Average predictive uncertainty (std): {:.2f}".format(np.mean(sigma_gp)))

                # Plot the GP predictions with uncertainty bands
                plt.figure(figsize=(8, 6))
                plt.scatter(X, y, color="blue", label="Ground Truth", alpha=0.6)

                # Sort the data for a smooth line plot
                sorted_idx = np.argsort(X.flatten())
                X_sorted = X.flatten()[sorted_idx]
                y_pred_sorted = y_pred_gp[sorted_idx]
                sigma_sorted = sigma_gp[sorted_idx]

                plt.plot(X_sorted, y_pred_sorted, 'r-', label="GP Prediction")
                plt.fill_between(X_sorted,
                                 y_pred_sorted - 2 * sigma_sorted,
                                 y_pred_sorted + 2 * sigma_sorted,
                                 color='r', alpha=0.2,
                                 label="Uncertainty Band (2σ)")
                plt.xlabel("Unsupervised HR Prediction")
                plt.ylabel("Ground Truth HR")
                plt.title("Gaussian Process Calibration with Uncertainty Bands")
                plt.legend()
                
                # Save the plot instead of showing it
                filename = f"{filename_id}_GP_uncertainty_plot.pdf"
                plt.savefig(filename)
                plt.close()
                print(f"GP uncertainty plot saved as: {filename}")
