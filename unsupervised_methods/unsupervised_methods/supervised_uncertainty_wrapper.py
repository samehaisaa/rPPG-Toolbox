"""
SupervisedUncertaintyWrapper module for rPPG-Toolbox.

This module implements a supervised uncertainty model that:
1. Takes a video and its ground truth PPG signal
2. Extracts PPG using CHROM and computes the error relative to ground truth
3. Extracts features from the video (SNR, RGB variances, etc.)
4. Trains a model to predict error/uncertainty from features
5. Provides a prediction function that returns signal with uncertainty estimates
"""
...
import cv2
from evaluation.post_process import _calculate_SNR, _calculate_fft_hr, _calculate_peak_hr, _compute_macc, _detrend, power2db
from scipy.signal import butter, filtfilt, welch
from unsupervised_methods.methods.CHROME_DEHAAN import CHROME_DEHAAN, process_video


class SupervisedUncertaintyWrapper:
    """
    Wrapper for estimating uncertainty in CHROM-extracted PPG signals using supervised learning.
    
    This class implements a model to predict uncertainty/error for each time point
    in the PPG signal based on features extracted from the video and signal,
    and the actual error compared to ground truth.
    """
    
    def __init__(self, fs=30, model_path=None):
        """
        Initialize the SupervisedUncertaintyWrapper.
    ...
    def extract_features(self, frames, ppg_signal):
        ...
        # 3. Calculate HR using both FFT and peak detection methods
        fs = self.fs
        try:
            hr_fft = _calculate_fft_hr(ppg_signal, fs=fs)
            hr_peak = _calculate_peak_hr(ppg_signal, fs=fs)
            signal_snr = _calculate_SNR(ppg_signal, hr_fft, fs=fs)
        except:
            hr_fft = 0
            hr_peak = 0
            signal_snr = 0
        ...
        shift_samples = int(0.2 * fs)  # 0.2 second shift
        ...
        try:
            ...
            [b_low, a_low] = butter(1, [0.6 / fs * 2, 1.0 / fs * 2], btype='bandpass')
            ...
            [b_hr, a_hr] = butter(1, [1.0 / fs * 2, 2.0 / fs * 2], btype='bandpass')
            ...
            [b_high, a_high] = butter(1, [2.0 / fs * 2, 3.3 / fs * 2], btype='bandpass')
            high_freq_signal = filtfilt(b_high, a_high, np.double(ppg_signal))
            high_freq_power = np.mean(np.square(high_freq_signal))
            
            # Calculate power spectral density
            f, pxx = welch(ppg_signal, fs=fs, nperseg=min(len(ppg_signal), 256))
            
            # Calculate signal power in heart rate band (0.75-2.5 Hz)
        ...
        for i in range(0, len(ppg_signal), hop_size):
            ...
            if len(ppg_window) >= int(fs * 1.5):  # Need at least 1.5 seconds for reasonable HR estimation
                try:
                    local_hr_fft = _calculate_fft_hr(ppg_window, fs=fs)
                    local_snr = _calculate_SNR(ppg_window, local_hr_fft, fs=fs)
                except:
            ...
            if len(ppg_window) >= int(fs * 1.0):  # Need at least 1 second
                try:
            ...
            window_features.extend([
                ...
                high_freq_power,
                zero_crossings,
                hr_power_ratio  # Ratio of HR power to total power
            ])
    ...
    def train(self, video_paths, gt_signals, output_model_path=None):
        """
        Train the supervised uncertainty model using videos and ground truth signals.
        ...
        for i, (video_path, gt_signal) in enumerate(zip(video_paths, gt_signals)):
            ...
            try:
                ...
                # Calculate absolute errors - this will be our target for supervised learning
        ...
        if not all_features or not all_errors:
            raise ValueError("No features or errors extracted from videos. Check the video paths and ground truth signals.")
        ...
        top_features = np.argsort(importances)[-10:]  # Get top 10 feature indices
